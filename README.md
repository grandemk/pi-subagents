...
- **FleetView** — Claude Code-style navigable list of `main` + every running subagent rendered below the editor (earliest-launched first). Press `↓` (or `←`) at an empty prompt to jump in, then `↑`/`↓` or `←`/`→` to change the selected agent's live, auto-updating conversation immediately; `Enter` does nothing, and `Esc` returns. Finished agents linger briefly before dropping out, and a viewer stays open through completion so you can read the final output. Toggle via `/agents → Settings → Fleet view`
- **Conversation viewer** — select any agent in `/agents` to open a live-scrolling view in pi's normal flow, rendered with the native transcript components (Markdown, user/assistant messages, tools, and bash output). It auto-follows new content; use `k`/`j` or Page Up/Page Down to scroll and pause, use `↑`/`↓` or `←`/`→` to move between agents (and return to the parent above/left of the first), and press `Esc` to return to the parent flow. Steer a running agent inline by pressing `Enter` to open a composer, typing, then `Enter` to send (`Esc` or an empty submit returns) — the message appears as a user message and redirects the agent after its current tool. Stop a still-running agent by pressing `x` (then `x` again to confirm) — both work for background agents too
