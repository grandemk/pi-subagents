...
<img width="600" alt="pi-subagents screenshot" src="https://github.com/grandemk/pi-subagents/raw/master/media/screenshot.png" />
...
- **FleetView** — Claude Code-style navigable list of `main` + every running subagent rendered below the editor (earliest-launched first). Press `↓` (or `←`) at an empty prompt to jump in, then `↑`/`↓` or `←`/`→` to change the selected agent's full-screen view immediately; `Enter` does nothing, and `Esc` returns. Finished agents linger briefly before dropping out, and the agent view stays open through completion so you can read the final output. Toggle via `/agents → Settings → Fleet view`
- **Agent view** — select any agent in `/agents` or FleetView to open its live session beside a persistent agent list. It uses pi's native transcript components (Markdown, user/assistant messages, tools, and bash output), auto-follows new content, and supports `k`/`j` or Page Up/Page Down scrolling. Use `↑`/`↓` or `←`/`→` to move between agents (and return to the parent above/left of the first), `Esc` to return to the parent flow, `Enter` to steer a running agent, and `x` twice to stop one. The parent transcript, editor, widgets and footer are restored when the view closes.
...
```bash
pi install npm:@madticstudio/pi-subagents
```
...
**Full guide:** [`docs/workflows.md`](https://github.com/grandemk/pi-subagents/blob/master/docs/workflows.md) — how the model writes the script for you, how to edit and re-run it, how to save one as a reusable named workflow, plus the complete `agent()` option reference, recipes and troubleshooting.
...
**Show cost** (`showCost`, default `false`): whether the subagent surfaces print an estimated cost beside their token counts — the widget (running *and* finished lines), [FleetView](#fleetview), the agent view, foreground results, `get_subagent_result`, and completion notifications:
...
**Agent view markdown** (`viewerMarkdown`, default `"assistant"`): how much of the [agent view](#ui)'s transcript is rendered as Markdown rather than shown verbatim.
...
src/
  ...
  ui/
    ...
    agent-view.ts          # Full-screen live agent session view
    viewer-keys.ts        # Agent-view scroll keys resolved through user keybindings
