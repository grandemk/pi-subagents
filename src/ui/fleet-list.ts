...
export class FleetList {
  ...
  handleKey(data: string): { consume?: boolean; data?: string } | undefined {
    ...
    if (matchesKey(data, "down") || matchesKey(data, "right")) {
      ...
      this.update();
      if (this.roster()[this.selectedIndex]?.kind === "agent") this.openSelected();
      return { consume: true };
    }
    ...
    // Agent selection is driven by arrows; Enter remains a no-op for agents.
    // Workflow rows retain Enter-to-open so multiple runs can be selected first.
    if (matchesKey(data, Key.enter)) {
      if (this.roster()[this.selectedIndex]?.kind === "workflow") this.openSelected();
      return { consume: true };
    }
